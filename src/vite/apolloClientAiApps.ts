...
function getVariableTypeName(
  operationName: string,
  operationType: "query" | "mutation"
) {
  return `${operationName}${operationType.charAt(0).toUpperCase()}${operationType.slice(1)}Variables`;
}
...
declare module "@apollo/client-ai-apps" {
  interface Register {
    toolName: ${toolNames.map((n) => `"${n}"`).join(" | ")};
  }
}
`.trimStart();
  }
...
// This file is auto-generated by @apollo/client-ai-apps. Do not edit manually.
import "@apollo/client-ai-apps";
import type { ${importedTypes.join(", ")} } from "./operation-types.js";

declare module "@apollo/client-ai-apps" {
  interface Register {
    toolName: ${toolNames.map((n) => `"${n}"`).join(" | ")};
    toolInputs: {
${toolInputEntries.join("\n")}
    };
  }
}
`.trimStart();
...
async function generateOperationTypes(
  ...
  // Only include documents that have operation definitions
  const operationDocs = sources.filter((doc) =>
    doc.definitions.some((d) => d.kind === Kind.OPERATION_DEFINITION)
  );
  ...
  // Strip @tool and @prefetch directives before passing to codegen
  const cleanedDocuments = operationDocs.map((doc) =>
    print(removeManifestDirectives(doc))
  );
...
export function apolloClientAiApps(
  ...
  async function generateManifest(environment?: Environment) {
    ...
    if (schema) {
      // Generate operation-types.d.ts via graphql-codegen
      ...
      if (currentOpTypesHash !== operationTypesHash) {
        ...
        const dest = path.resolve(root, operationTypesOutFile);
        fs.mkdirSync(path.dirname(dest), { recursive: true });
        fs.writeFileSync(dest, opTypesContent);
      }

      // Generate register.d.ts with toolInputs
