...
describe("tool input types", () => {
  ...
  test("generates register.d.ts with toolInputs when schema is provided", async () => {
    ...
    expect(content).toMatchInlineSnapshot(`
      ...
      import type { CreateTodoVariables } from "./operation-types.js";

      declare module "@apollo/client-ai-apps" {
        interface Register {
          toolName: "CreateTodo";
          toolInputs: {
            "CreateTodo": CreateTodoVariables;
          };
        }
      }"
    `);
  ...
  test("tool with no extraInputs uses only the Variables type", async () => {
    ...
    expect(content).toMatchInlineSnapshot(`
      ...
      import type { DeleteTodoVariables } from "./operation-types.js";

      declare module "@apollo/client-ai-apps" {
        interface Register {
          toolName: "DeleteTodo";
          toolInputs: {
            "DeleteTodo": DeleteTodoVariables;
          };
        }
      }"
    `);
